function createCanvas(element) {
  ...
  var outer_div = element
    .append("div")
    .styles({ width: element_width, height: element_height });
  screen_svg = {};
  screen_svg.canvas = outer_div
    .append("svg")
  ...
  screen_svg.canvas
    .append("image")
    .attrs({
      x: temp_pos.x - 0.5 * refresh_icon_size,
      y: temp_pos.y - 0.5 * refresh_icon_size,
      width: refresh_icon_size,
      height: refresh_icon_size,
      "xlink:href": "../../Images/settings.svg",
    });

  refresh_icon_circle = screen_svg.canvas
    .append("circle")
    .styles({
      fill: "gray",
      "fill-opacity": 0,
  ...
  refresh_icon_circle.on("click", function () {
    swal({
      title: "Settings",
      html:
        '<input id="settings_show_decimals" type="checkbox" ' +
        (screen_svg.settings.show_decimals ? "checked = ''" : "") +
        " /> Show Decimals",
      showCloseButton: true,
      confirmButtonText: "Done",
    }).then((result) => {
      screen_svg.settings.show_decimals = document.getElementById(
        "settings_show_decimals"
      ).checked;
  ...
  var temp_text = screen_svg.canvas
    .append("text")
    .attrs({ x: 0.5 * element_width, y: 0.97 * element_height })
    .styles({ "font-size": 14 });

  temp_text.append("tspan").styles({ color: "gray" }).text("- designed by");
    ...
    if (d3.event.touches.length == 2) {
      ...
      const zx =
        ((touch1.pageX + touch2.pageX) / 2 -
          screen_svg.canvas.node().getBoundingClientRect().left) /
        screen_dpi;
      const zy =
        ((touch1.pageY + touch2.pageY) / 2 -
          screen_svg.canvas.node().getBoundingClientRect().top) /
        screen_dpi;
