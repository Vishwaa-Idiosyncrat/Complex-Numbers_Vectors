...
createVector.prototype.resolve_vector = function(){
  ...
  this.text_3.data.tspans[1].value = "_r";
  this.text_3.data.tspans[6].value = "_θ";
...
createVector.prototype.recombine_vector = function(){
  ...
  this.text_1.data.tspans[0].value = `${this.symbol}`;
  ...
  for(i in this.parent.vector_list){
    if(i != index && this.vector_mode == "polar" && this.parent.vector_list[i].vector_mode == "polar"){
      ...
      if(this.position == "resultant"){
        const symbol1 = complexSymbols[this.addition_data.patner.vectorID % 4];
        const symbol2 = complexSymbols[this.vectorID % 4];
        this.text_1.data.tspans[0].value = `(${symbol1}+${symbol2})`;
        this.text_2.data.tspans[0].value = `(${symbol1}+${symbol2})_r`;
        this.text_3.data.tspans[0].value = `(${symbol1}+${symbol2})_θ`;
      }
    }
...
createVector.prototype.checkForAddition = function(){
  // Changed addition notation to complex number format
  const complexSymbols = ['z', 'w', 'u', 'v'];
...
createVector.prototype.resolve_vector = function(){
  // Update component labels to complex number format
  this.text_3.data.tspans[1].value = "_r";
  this.text_3.data.tspans[6].value = "_θ";
}
