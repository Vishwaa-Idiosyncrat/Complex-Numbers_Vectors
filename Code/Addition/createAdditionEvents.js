...
var create_Resultant_Events = function () {
  ...
  object.dispatch.on("long_press", function () {
    if (
      ...
      resultant.change_mode_allowed == true
    ) {
      object.toggle_addition_mode();
    }
    if (object.manipulationMode == false) {
      ...
      object.done_addition_button.image
        ...
        .attrs({
          x:
            resultant.cx +
            object.done_addition_button.posX -
            0.5 * object.done_addition_button.size,
          y:
            resultant.cy +
            object.done_addition_button.posY -
            0.5 * object.done_addition_button.size,
          width: object.done_addition_button.size,
          height: object.done_addition_button.size,
        });
    }
  });
  ...
  // In create_Resultant_Events() function
  resultant.vector_resolve_rect.on("touchstart", function (d) {
    // Update text labels for all vectors during interaction
    d3.selectAll(".vector-text").each(function (vector) {
      const r = radius_scale(vector.r).toFixed(1);
      const θ = vector.angle_deg.toFixed(1);
      d3.select(this).html(`
      ${vector.symbol}:<br>
      <tspan dx="0" dy="1.2em">${vector.symbol}_r = ${r}</tspan><br>
      <tspan dx="0" dy="1.2em">${vector.symbol}_θ = ${θ}°</tspan>
    `);
    });

    // Original touchstart logic
    tempArray = [];
    temp_resolved = false;
    if (
      d3.event.targetTouches.length == 2 &&
      object.addition_mode == "triangle" &&
      object.componentized == false &&
      d.resolution_allowed
    ) {
  ...
  resultant.vector_resolve_rect.on("touchmove", function (d) {
    if (d3.event.targetTouches.length == 2) {
      ...
      temp_speed =
        (dist_1 - dist_2) /
        (tempArray[0].timeStamp - tempArray[tempArray.length - 1].timeStamp);
      if (
        temp_speed > 0.4 &&
        !temp_resolved &&
        d.resolution_allowed &&
  ...
  resultant.centre_control_circle_1.on("touchstart", function () {
    ...
    object.timer = setTimeout(function () {
      object.dispatch.call("long_press", this, {});
    }, 600);
    if (object.manipulationMode == true && object.componentized == true) {
      resultant.recombine_vector_circle = resultant.parent.canvas
        .append("circle")
        .styles({ fill: resultant.gray_color, "fill-opacity": 0.2 })
        .attrs({
          cx: resultant.xComponent_coordinate,
          cy: resultant.yComponent_coordinate,
          r: resultant.control_circle_radius,
        })
        .on("touchstart", function () {
          clearTimeout(object.timer);
          object.recombine_added_vector();
        });
    }
  });
  ...
  resultant.centre_control_circle_1.on("touchend", function () {
    ...
    clearTimeout(object.timer);
    if (resultant.recombine_vector_circle) {
      resultant.recombine_vector_circle.remove();
    }
  ...
  resultant.centre_control_circle_1.on("touchmove", function () {
    ...
    if (button.shown == true) {
      var temp_dist = distpoints(
        d3.event.targetTouches[0].pageX,
        d3.event.targetTouches[0].pageY,
        resultant.cx + button.posX,
        resultant.cy + button.posY
      );
      if (temp_dist < 0.5 * button.size) {
        button.active = true;
      } else {
        button.active = false;
      }
  ...
  // In movement circle drag handler
  drag_movement_circle.on("drag", function (d) {
    if (d3.event.sourceEvent.type == "touchmove") {
      // Update position
      d.cx =
        d.temp_pos.cx +
        (d3.event.sourceEvent.targetTouches[0].pageX - d.temp_pos.x);
      d.cy =
        d.temp_pos.cy +
        (d3.event.sourceEvent.targetTouches[0].pageY - d.temp_pos.y);

      // Convert to complex plane coordinates (z = x + iy)
      const real = (d.cx / screen_dpi).toFixed(1);
      const imag = (d.cy / screen_dpi).toFixed(1);

      // Update complex position display
      d3.select("#complexPosition")
        .style("left", `${d.cx + 20}px`)
        .style("top", `${d.cy}px`)
        .html(`z = ${real} ${imag >= 0 ? "+" : "-"} i${Math.abs(imag)}`);
