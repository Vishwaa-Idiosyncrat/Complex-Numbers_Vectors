...
var recombine_added_vector = function(){
  ...
  resultant.vector_head_circle
    ...
    setTimeout(function(){
      // Restore complex number symbols
      [vector_1, vector_2, resultant].forEach(vector => {
        vector.text_1.data.tspans[0].value = `${vector.symbol}`;
        
        // Update main complex number display
        vector.text_1.data.tspans[0].value = 
          `\\( \\overline{${vector.symbol}} \\)`;  // Complex conjugate notation
        
        // Remove component equations
        if(vector.equationDiv) vector.equationDiv.remove();
        
        // Add polar form equation
        vector.equationDiv = d3.select('body').append('div')
          .styles({
            'position': 'absolute',
            'font-size': '1.2em',
            'color': vector.vector_color,
            'left': `${vector.cx + 20}px`,
            'top': `${vector.cy}px`
          })
          .html(`\\( ${vector.symbol} = ${vector.r.toFixed(1)}e^{i${vector.angle_deg.toFixed(1)}^\\circ} \\)`);
        
        MathJax.Hub.Queue(["Typeset", MathJax.Hub]);
      });
      
      // Update resultant display
      resultant.text_1.data.tspans[0].value = 
        `\\( ${vector_1.symbol} + ${vector_2.symbol} \\)`;
...
var resolve_added_vector = function(){
  ...
  resultant.yProjection_circle
    ...
    setTimeout(function(){
      // Update component labels for all vectors to complex notation
      [vector_1, vector_2, resultant].forEach(vector => {
        vector.text_3.data.tspans[1].value = "_r";
        vector.text_3.data.tspans[6].value = "_θ";
        vector.update_text();
        
        // Add complex number equation display
        vector.equationDiv = d3.select('body').append('div')
          .styles({
            'position': 'absolute',
            'font-size': '1.2em',
          ...
          .html(`\\( ${vector.symbol} = ${vector.symbol}_r e^{i${vector.symbol}_θ} \\)`);
        
        MathJax.Hub.Queue(["Typeset", MathJax.Hub]);
      });
      
      // Update resultant equation
      resultant.text_3.data.tspans[3].value = 
        `${vector_1.symbol}_r + ${vector_2.symbol}_r`;
      resultant.text_3.data.tspans[8].value = 
        `${vector_1.symbol}_θ + ${vector_2.symbol}_θ`;
