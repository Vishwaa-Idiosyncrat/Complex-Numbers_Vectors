createVector.prototype.addVectors = function () {
  ...
  object.done_addition_button = {
    size: 5 * screen_size,
    size_big: 8 * screen_size,
  };
  ...
  object.done_addition_button.image = object.resultant.parent.canvas
    .append("image")
    .attrs({
      "xlink:href": "../../Images/done.svg",
      width: 0,
      height: 0,
      x: object.resultant.cx,
      y: object.resultant.cy,
    });

  object.vector_1.vector_line_dotted.styles({
    "stroke-width": 0.3 * screen_size,
    "stroke-dasharray": "6,4",
  });
  object.vector_2.vector_line_dotted.styles({
    "stroke-width": 0.3 * screen_size,
    "stroke-dasharray": "6,4",
  });

  object.resultant.vector_resolve_rect.attrs({
    x: 0,
    y: -12 * screen_dpi,
    width: object.resultant.r,
    height: 24 * screen_dpi,
  });

  object.vector_1_dotted_line = object.vector_1.container
    .append("line")
    .styles({
      stroke: object.vector_1.vector_color,
      "stroke-width": 0.3 * screen_size,
  ...
  object.vector_2_dotted_line = object.vector_1.container
    .append("line")
    .styles({
      stroke: object.vector_2.vector_color,
      "stroke-width": 0.3 * screen_size,
      "stroke-dasharray": "6,3",
      display: "none",
    })
  ...
  object.resultant.movement_circle = object.resultant.parent.canvas
    .append("circle")
    .styles({ fill: "white", "fill-opacity": 0, display: "none" })
    .data([object.resultant]);
  object.resultant.centre_control_circle_1 = object.resultant.parent.canvas
    .append("circle")
    .styles({ fill: object.resultant.vector_color, "fill-opacity": 0 });
  ...
  for (i = 0; i < object.vector_1.parent.vector_list.length; i++) {
    if (
      this.vectorID == object.vector_1.parent.vector_list[i].vectorID ||
      this.addition_data.patner_ID ==
        object.vector_1.parent.vector_list[i].vectorID
    ) {
      object.vector_1.parent.vector_list.splice(i, 1);
      i--;
    }
  }
  ...
  object.setup_Addition_View(
    object.vector_1,
    object.vector_2,
    object.resultant
  );
  object.create_Addition_Events(
    object.vector_1,
    object.vector_2,
    object.resultant
  );
  ...
  var symbol_1 = object.vector_1.symbol,
    symbol_2 = object.vector_2.symbol,
    symbol_r = object.resultant.symbol;
  object.div = d3
    .select("body")
    .append("div")
    .styles({ "font-size": object.font_size_normal, position: "absolute" });
  object.div
    .append("text")
    .html(`\\( ${symbol_r} = ${symbol_1} + ${symbol_2} \\)<br>`)
    .styles({ "font-size": object.font_size_normal });
  object.div
    .append("text")
    .html(`\\( ${symbol_r}_r = ${symbol_1}_r + ${symbol_2}_r \\)<br>`);
  object.div
    .append("text")
    .html(`\\( ${symbol_r}_θ = ${symbol_1}_θ + ${symbol_2}_θ \\)`);
...
var update_Added_vectors = function () {
  ...
  this.vector_1_dotted_line.attrs({
    x1: this.vector_2.xComponent_length,
    y1: -this.vector_2.yComponent_length,
    x2: this.vector_2.xComponent_length + this.vector_1.xComponent_length,
    y2: -this.vector_2.yComponent_length - this.vector_1.yComponent_length,
  });
  this.vector_2_dotted_line.attrs({
    x1: this.vector_1.xComponent_length,
    y1: -this.vector_1.yComponent_length,
    x2: this.vector_2.xComponent_length + this.vector_1.xComponent_length,
    y2: -this.vector_2.yComponent_length - this.vector_1.yComponent_length,
  });

  this.vector_1.xComponent_line.attrs({
    x1: 0,
    y1: 0,
    x2: this.vector_1.xComponent_length,
    y2: 0,
  });
  this.vector_1.yComponent_line.attrs({
    x1: 0,
    y1: 0,
    x2: 0,
    y2: -this.vector_1.yComponent_length,
  });
  this.vector_2.xComponent_line.attrs({
    x1: 0,
    y1: this.vector_1.yComponent_length,
    x2: 0 + this.vector_2.xComponent_length,
    y2: this.vector_1.yComponent_length,
  });
  this.vector_2.yComponent_line.attrs({
    x1: -this.vector_1.xComponent_length,
    y1: 0,
    x2: -this.vector_1.xComponent_length,
    y2: 0 - this.vector_2.yComponent_length,
  });

  var xComponent_length =
    this.vector_1.xComponent_length + this.vector_2.xComponent_length;
  var yComponent_length =
    this.vector_1.yComponent_length + this.vector_2.yComponent_length;
  var r = Math.sqrt(
    xComponent_length * xComponent_length +
      yComponent_length * yComponent_length
  );
  var angle_rad = Math.atan2(yComponent_length, xComponent_length);
  this.resultant.r = r;
  this.resultant.angle_rad = angle_rad;

  this.resultant.movement_circle.attrs({
    cx: this.resultant.cx,
    cy: this.resultant.cy,
    r: this.resultant.r,
  });
  this.resultant.centre_control_circle_1.attrs({
    cx: this.resultant.cx,
    cy: this.resultant.cy,
    r: this.resultant.control_circle_radius,
  });
  this.resultant.update();

  this.div.styles({
    top: this.resultant.cy + this.resultant.r + 10,
    left: this.resultant.cx - 0.5 * parseInt(this.div.style("width")),
  });
};
...
var setup_Addition_View = function () {
  ...
  vector_1.xAxis.transition().duration(1000).delay(0).styles({ opacity: 0 });
  vector_1.yAxis.transition().duration(1000).delay(0).styles({ opacity: 0 });
  vector_1.circle
    .transition()
    .duration(1000)
    .delay(0)
  ...
  vector_2.xAxis.transition().duration(1000).delay(0).styles({ opacity: 0 });
  vector_2.yAxis.transition().duration(1000).delay(0).styles({ opacity: 0 });
  vector_2.circle
    .transition()
    .duration(1000)
    .delay(0)
  ...
  var xComponent_length =
    vector_1.xComponent_length + vector_2.xComponent_length;
  var yComponent_length =
    vector_1.yComponent_length + vector_2.yComponent_length;
  var r = Math.sqrt(
    xComponent_length * xComponent_length +
      yComponent_length * yComponent_length
  );
...
createVector.prototype.create_Resultant = function () {
  // In addition.js create_Resultant()
  const complexSymbols = ["z", "w", "u", "v"];
  temp_vector.symbol = complexSymbols[temp_vector.vectorID % 4];
...
createVector.prototype.create_Vector_2 = function (resultant) {
  ...
  temp_vector.vector_line.attrs({
    "marker-end": "url(#arrow_component_" + temp_vector.vectorID + ")",
  });
  return temp_vector;
};
...
// Change the equation creation in addVectors()
